...
export function calculateOverallEvaluation(data: StoreData): OverallEvaluation {
  ...
  const level = score >= 85 ? '成熟稳定' : score >= 70 ? '发展良好' : score >= 55 ? '改进中' : '需要积累证据';
  ...
  return Array.from(buckets.entries())
    ...
    .map(([month, value]) => {
      const monthData: StoreData = {
        ...
        teacherIssueJobs: data.teacherIssueJobs.filter((item) => item.createdAt.slice(0, 7) <= month),
...
## 三、教师发展画像
- 整体评分：${overall.score}/100（${overall.level}）
